...
__global__ 
void kernel(t_pbc *pbc,real beta_i,real *beta,rvec x_i,rvec *x,real rmax2,rvec qvec,real *result,int n,int N) {
...
extern void double_sum(t_pbc *pbc,real *beta,rvec *x,int n,real rmax2,rvec *qvec,int nbinq,real *temp_method) {
  ...
  for (i=0;i<n;i++) {  
    for (q=0;q<nbinq;q++) {
      ...
      kernel<<<grid_size,block_size>>>(pbc_d,beta[i],beta_d,x_d[i],x_d,rmax2,qvec_d[q],result_d,n,N);
...
/*
extern void calc_beta(t_pbc *pbc,int natoms,int n,int *ind,rvec *x,real norm_x,real norm_z,rvec pol_out,rvec pol_in1,
                      rvec pol_in2,real *bete_mol_1d,real *beta) {
  ...
  // Allocate Device Memory
  cudaMalloc(&pbc_d,sizeof(t_pbc));
  cudaMalloc(&ind_d,sizeof(int)*n);
  cudaMalloc(&x_d,sizeof(rvec)*natoms);
  cudaMalloc(&beta_d,sizeof(real)*n);

  // Copy memory from Host to Device
  cudaMemcpy(pbc_d,pbc,sizeof(t_pbc),cudaMemcpyHostToDevice);
  cudaMemcpy(ind_d,ind,sizeof(int)*n,cudaMemcpyHostToDevice);
  cudaMemcpy(x_d,x,sizeof(rvec)*natoms,cudaMemcpyHostToDevice);

  // Launch cuda kernel with n threads
  beta_kernel<<<grid_size,block_size>>>(pbc_d,ind_d,x_d,);
  // Copy result back from Device to Host 
  cudaMemcpy(beta,beta_d,sizeof(real)*n,cudaMemcpyDeviceToHost);
